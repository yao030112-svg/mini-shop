<template>
  <view class="cart">
    <!-- 自定义导航栏 -->
    <view class="cart__navbar">
      <view class="cart__navbar-back" @click="goBack">
        <text class="cart__navbar-back-icon">‹</text>
      </view>
      <text class="cart__navbar-title">购物车</text>
      <view class="cart__navbar-placeholder"></view>
    </view>
    ...
    <view v-if="cartList.length > 0" class="cart__footer">
      ...
      <view class="cart__footer-right">
        ...
        <view class="cart__footer-btn" :class="{ 'cart__footer-btn--disabled': selectedCount === 0 }" @click="handleCheckout">
          <text class="cart__footer-btn-text">去结算({{ selectedCount }})</text>
        </view>
      </view>
    </view>
  </view>
...
.cart {
  ...
  padding-bottom: 140rpx; // 增加底部留白，避免被 tabbar 遮挡
}
...
.cart__footer {
  ...
  z-index: 999; // 提高 z-index，确保在 tabbar 之上
